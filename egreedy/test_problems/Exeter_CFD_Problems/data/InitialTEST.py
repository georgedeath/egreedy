...
class InitialTEST(object):
    ...
    def __new__(cls,*args,**kwargs):
        obj=super(InitialTEST,cls).__new__(cls)
        # the following lines were not in the original class
        # these are used to add keyword class attributes to the derived class
        obj.case_path = kwargs.get('case_path', '.')
        obj.size_class = kwargs.get('size_class', 'small')
        obj.min_run_time = kwargs.get('min_run_time', 100)
        ##################################################
        obj.__parameters={}
    ...
    def __recursiveInit(self,theClass,called):
        ...
        for b in theClass.__bases__:
            if b not in [object,InitialTEST]:
    ...
    def sizeClassString(self):
        return ", ".join(["%s = %ds"% t for t in InitialTEST.timeoutDefinitions])
    ...
    def setTimeout(self,quiet=False):
        ...
        try:
            self.timeout=dict(InitialTEST.timeoutDefinitions)[self["sizeClass"]]
            index=-1
            for i,v in enumerate(InitialTEST.timeoutDefinitions):
                if v[0]==self["sizeClass"]:
                    index=i
                    break
            if index>2:
                self.proposedSizeClass,self.toSmallTimeout=InitialTEST.timeoutDefinitions[index-2]
        except KeyError:
            self.fatalFail("sizeClass is specified as",self["sizeClass"],
                           ". Valid values are with their timeout values",
                           self.sizeClassString())
            self.timeout=dict(InitialTEST.timeoutDefinitions["unknown"]) # just in case that we continue
    ...
    def __doInit(self,
        ...
        self.__setParameterAsUsed(["solver","minimumRunTime",
                                   "referenceData","tailLength","headLength", "casePath"])#originalCase","
        print (casePath)
    ...
    def run(self):
        ...
        if self.doPreparation:
            ...
            self.casePrepare()
            # becareful!
            self.status("Preparing case - tolerance")
            self.casePrepare_tolerance()
        ...
        if self.doSimulation:
            ...
            self.__runInfo=dict(self.execute(self.solver).getData()) # check
            #self.writeRunInfo()
            print_()
        ...
        if self.doPrintRunInfo:
            ...
            printer.pprint(self.__runInfo) # check
    ...
    def workDir(self):
        try:
            return os.environ["PYFOAM_InitialTEST_WORKDIR"]
        except KeyError:
            if not hasattr(self,"__checkWorkDir"):
                self.__checkWorkDir=None
                self.warn("No environment variable PYFOAM_InitialTEST_WORKDIR defined. Using current directory")
            return path.curdir

    # make configuration-dependent
    def dataDir(self):
        try:
            return os.environ["PYFOAM_InitialTEST_DATADIR"]
        except KeyError:
            if not hasattr(self,"__checkDataDir"):
                self.__checkDataDir=None
                self.warn("No environment variable PYFOAM_InitialTEST_DATADIR defined. Using current directory")
            return path.curdir
    ...
    def fvSolution(self):
        """Access a representation of the fvSolution of the case"""
        if not hasattr(self,"_fvSolution"):
            self._fvSolution=ParsedParameterFile(self.solution().fvSolution())
    ...
    @isCallback
    def casePrepare_tolerance(self):
        """Callback to prepare the case. Default behaviour is to do
        nothing"""
        pass
